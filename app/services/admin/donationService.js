const DonationType = require("../../models/admin/donationModel");
...
const updateDonationTypeDetails = async (req) => {
  ...
  const existingDonationType = await DonationType.findOne({
    _id: req.params.id,
  });
...
const getDonationTypeDetailsByType = async (req, res) => {
  ...
  const donationType = await DonationType.findOne({
