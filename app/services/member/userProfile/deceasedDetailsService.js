const DeceasedDetails = require("../../../models/member/userProfile/deceasedDetailsModel");
...
const getDeceasedDetailsByUserId = async (req) => {
  ...
  const deceased = await DeceasedDetails.findOne({
...
const getDeceasedListByUserId = async (req) => {
  ...
  const deceasedlist = await DeceasedDetails.find({
    userId: req.params.userId,
  });
  if (!deceasedlist) {
    const data = {
...
const deleteDeceasedDetails = async (req, res) => {
  ...
  const Deceased = await DeceasedDetails.deleteOne({
    _id: req.params.deceasedId,
  });
