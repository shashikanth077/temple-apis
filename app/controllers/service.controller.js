...
const {
    getAllServices, getServicesByGodId, addServiceDetails, 
    updateServiceDetailsByGodId, inactivateServiceByServiceId,
    inactivateServiceByGodId,getServiceByServiceId
  ...
  exports.getAllServicesController = async (req, res, next) =>  {
    try {
        ...
        res.status(500).json({ success: false, error: 'Internal server error (getAllServices)' });
    }
  };
  ...
  exports.getServiceByServiceIdController = async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Internal server error (getServiceByServiceIdController)" });
    }
  };
  ...
  exports.addServiceDetailsController = async (req, res, next) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Internal server error (addServiceDetails)' });
    }
  };
  ...
  exports.inActivateServiceByServiceIdController = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Something went wrong please try again (inActivateServiceByServiceIdController)' });
    }
};
    ...
    exports.inActivateServiceByGodIdController = async (req, res) => {
        try {
            ...
            //logger.error('deleteGodDetails Error:', error);
            res.status(500).json({ success: false, error: 'Something went wrong please try again (inActivateServiceByGodIdController)' });
        }
  };
